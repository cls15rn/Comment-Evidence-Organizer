# 악플 증거 정리 도구 (working title)
> Comment Evidence Organizer — 혹시 모를 상황에 대비해, 사용자가 지목한 악플을 사라지기 전에 봉인해 두는 증거 정리 도구

<!-- TODO: 최종 프로젝트명 확정 후 제목·리포지토리명 통일 -->

<!-- 배지(Badges): 코드/배포 확정 후 추가. 예시 자리만 잡아둠 -->
...
<!-- TODO: Streamlit 데모 배포 후 링크 추가 (Streamlit Community Cloud, 무료·공개) -->
<!-- TODO: 시연 GIF / 스크린샷 추가 — 포트폴리오에서 라이브보다 영상이 더 자주 소비됨 -->
...
<!-- TODO: 리포지토리에 이미지 넣고 상대경로로 교체 (docs/ 폴더 예정) -->
![통합 설계도](docs/idea3_diagram.png)

![다섯 박자](docs/idea3_five_beats.png)

- 통합 흐름도: `idea3_diagram.svg` / `.png`
...
**요구 사항 (Requirements):** `TODO` (Python 버전, OCR 엔진, 모델 관련 의존성 등)

---

## 사용법 (Usage)

<!-- TODO: 데모 완성 후 실제 실행 커맨드/화면 흐름으로 채움 -->
...
기본 흐름: 캡처 업로드 → 자동 봉인·분류 → 세 가지 뷰로 출력 → 필요한 형식으로 내보내기.
...
```
├─ app/                        # Streamlit 앱 (추후 도메인 연동)
│   └─ app.py                  # ✅ 파이프라인 조립 + 저장≠열람 UI
...
- **작성된 파일 (현재) — 파이프라인 뼈대 완성:**
  ...
  - `src/classify.py` — 형식적 특징 관찰 + 참고 조항 유형 연결(판정 아님) + 면책. 작은/큰 모델 2단 구조(`screen`→`observe_features`→`map_provisions`), `[MODEL]` 슬롯이 실제 모델 교체 지점.
  - `src/views.py` — 한 데이터 → 증거표(CSV utf-8-sig + PDF reportlab 한국어 CID, 없으면 HTML 폴백)·타임라인(반복 '관찰', 동일인 추정 X)·죄목별(근거+면책).
  - `app/app.py` — 위를 잇는 Streamlit 데모: 캡처→OCR초안→교정→봉인→분류→세 뷰. 봉인 후 표지만 보이고 원문은 펼칠 때만(저장≠열람).
...
- [ ] Streamlit 데모: 캡처 → OCR → SHA-256 봉인 → (작은/큰 모델) 분류 → 세 가지 뷰 출력
- [ ] 무료 배포 (Streamlit Community Cloud)
- [ ] **비용 제약 결정: 큰 모델을 로컬/오픈소스 vs 외부 API 중 어디서 돌릴지** (비용 상한선 아래에서)
- [ ] (확장) TSA 시점 보증 연동
- [ ] (확장) 공유(share sheet) 입력 경로
...
- <!-- TODO: OCR 정확도, 지원 플랫폼, 모델 오분류 가능성 등 실측 후 구체화 -->
...
- 이 README는 아이디어 3 전용 채팅에서 설계 확정 후 작성한 뼈대다.
- 채울 수 있는 것(개요·문제·기능·아키텍처·설계결정·Non-Goals·로드맵)은 채웠고,
  코드가 있어야 정해지는 것(폴더구조·설치·실행·의존성·데모)은 TODO로 남겼다.
- 억지로 지어내지 말 것 — 코드와 안 맞으면 거짓이 된다. 제작하며 TODO를 실측으로 교체.
